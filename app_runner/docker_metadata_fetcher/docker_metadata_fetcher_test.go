...
var _ = Describe("DockerMetaDataFetcher", func() {
	Describe("FetchMetadata", func() {
		...
		BeforeEach(func() {
			...
			server.RouteToHandler("GET", "/v1/repositories/ouruser/ourrepo/images", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			server.RouteToHandler("GET", "/v1/repositories/ouruser/ourrepo/tags", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
				w.Write([]byte(`{
								   "latest": "id-1",
								   "missing-info": "id-2"
							   }`))
			...
			server.RouteToHandler("GET", "/v1/images/id-1/json", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			server.RouteToHandler("GET", "/v1/images/id-2/json", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
				w.Header().Add("X-Docker-Size", "789")
		...
		Context("with missing metadata", func() {
			It("returns an error", func() {
				...
				repoName := registryHost + "/ouruser/ourrepo"
				_, err := dockerMetaDataFetcher.FetchMetadata(repoName, "missing-info")
