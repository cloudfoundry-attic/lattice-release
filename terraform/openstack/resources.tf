...
resource "openstack_compute_instance_v2" "lattice-coordinator" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "sudo sh -c 'echo \"CONSUL_SERVER_IP=${openstack_compute_instance_v2.lattice-coordinator.private_ip}\" >> /var/lattice/setup/lattice-environment'",
            "sudo sh -c 'echo \"SYSTEM_DOMAIN=${openstack_compute_instance_v2.lattice-coordinator.public_ip}.xip.io\" >> /var/lattice/setup/lattice-environment'",
        ]
    }
